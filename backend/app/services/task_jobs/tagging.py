...
The Celery entrypoint (``apply_tags_to_contacts``) is a thin wrapper around the
``_apply_tags_to_contacts`` coroutine so that the tagging logic is directly
...
async def _apply_tags_to_contacts(
    db: AsyncSession,
    uid: uuid.UUID,
    contact_ids: list[str] | None = None,
) -> dict:
    """Apply approved taxonomy tags to contacts in bulk for user ``uid``."""
    # Load taxonomy
...
def apply_tags_to_contacts(self, user_id: str, contact_ids: list[str] | None = None) -> dict:
    ...
    async def _runner() -> dict:
        async with task_session() as db:
            return await _apply_tags_to_contacts(db, uid, contact_ids)
