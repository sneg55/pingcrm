...
The Celery entrypoint (``scan_meeting_preps``) is a thin wrapper around the
``_scan_meeting_preps`` coroutine so the sync logic is directly unit-testable
against a real ``AsyncSession`` without spinning up a Celery broker or the
``task_session`` machinery.
"""
...
async def _scan_meeting_preps(db: AsyncSession, redis_client) -> dict:
    """Scan for meetings starting in ~30 minutes and email prep briefs.

    Returns a status dict with sent/skipped/errors counts. Per-user / per-meeting
    failures are recorded in ``errors`` and (for auth issues) written as a
    ``Notification`` row; the function does not raise for those.

    ``redis_client`` is taken as an argument so tests can inject a fake. It is
    used synchronously for dedup checks — the call sites here are intentionally
    sync calls inside this async function, matching the original behaviour.
    """
    ...
    for user_id in all_user_ids:
        ...
        for meeting in meetings:
            ...
            if result is True:
                redis_client.set(dedup_key, "1", ex=86400)  # 24h TTL
                sent += 1
...
def scan_meeting_preps(self) -> dict:
    ...
    async def _runner() -> dict:
        async with task_session() as db:
            return await _scan_meeting_preps(db, r)
